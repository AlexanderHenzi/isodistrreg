...
<p align="center">
<strong>Available on
<a href="https://cran.r-project.org/package=isodistrreg">CRAN</a></strong>
</p>
...
`install.packages("isodistrreg")`. If the installation fails on Linux
because of the error *Vignette re-building failed*, try installing with
`build_vignettes = FALSE` or make sure that texinfo is installed (e.g.,
`sudo apt-get install texinfo`).

The development version can be built from source as follows:

1.  You need [a working rust
    installation](https://extendr.rs/rextendr/articles/setting_up_rust.html)
2.  Install the build dependencies in your R environment:
    `install.packages(c("rextendr", "tomledit", "fs"))`
